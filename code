    ...
    if not is_raining :
        ...
        if abs(time.ticks_diff(time.ticks_ms(), last_distance_check_time)) <= 5000:
            if distance_change >= 30:
                print("Water level has risen by more than 30mm within 5 seconds.")
                lcd.move_to(0, 3)
                ...
                smtp.write("Water level has risen by more than 30mm within 5 seconds.")
                smtp.send()
                smtp.quit()
            elif distance_change >= 20:
                print("Water level has risen by more than 20cm within 5 seconds.")
                lcd.putstr(0, 3)
                lcd.putstr("Warning             ")
                smtp = umail.SMTP('smtp.gmail.com', 465, ssl=True) # Gmail's SSL port
                smtp.login(sender_email, sender_app_password)
                ...
                smtp.write("From:" + sender_name + "<"+ sender_email+">\n")
                smtp.write("Subject:" + email_subject + "\n")
                smtp.write("Water level has risen by more than 20mm within 5 seconds.")
                smtp.send()
                smtp.quit()
    else:
        initial_distance = 0
